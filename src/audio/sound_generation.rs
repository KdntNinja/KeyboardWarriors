...
pub fn setup_piano_notes(
    ...
    for (note_name, frequency) in &notes {
        ...
        if !file_path.exists() {
            ...
            writer.write_all(&((36 + audio_data.len() * 2) as u32).to_le_bytes()).unwrap(); // File size - 8
            ...
            writer.write_all(&((audio_data.len() * 2) as u32).to_le_bytes()).unwrap(); // Chunk size
        ...
        sound_handles.handles.push((note_name.to_string(), Handle::<AudioSource>::default()));
    }
    
    // Save notes to resource for use in sound generation
    commands.insert_resource(PianoNotes {
        notes: notes.iter().map(|(name, freq)| (name.to_string(), *freq)).collect(),
    });
}

// System to manually load audio files after they've been generated
pub fn load_audio_files(
    mut sound_handles: ResMut<SoundHandles>,
